...
- 需要在方法上加上 `#[Test]` attribute 讓 PHPUnit 識別（使用 PHP 8+ attribute 語法，不使用 `@test` 註解）
- 需要 import：`use PHPUnit\Framework\Attributes\Test;`

### 測試原則
- **測試核心邏輯，不測試框架本身**：只測試我們定義的業務邏輯和驗證規則，不測試 Laravel 框架內建功能（如 `required`、`email` 規則的各種邊界情況）
- **避免過度測試**：不測試框架已保證的行為（例如：`required` 規則會同時處理「缺失」和「空字串」）
- **測試意圖清晰**：每個測試應該只驗證一個明確的行為
- **提取重複代碼**：使用 helper method 減少測試中的重複代碼（如 `getRules()`）

### 何時需要測試框架行為？
以下情況**可以**測試框架行為：
- 自定義驗證規則（Custom Validation Rule）
- 複雜的業務邏輯驗證（如條件式驗證、跨欄位驗證）
- 框架版本升級後需要確認行為未改變（回歸測試）
